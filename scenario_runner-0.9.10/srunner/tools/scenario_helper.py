...
def get_crossing_point(actor):
    ...
    while not wp_cross.is_intersection:  # till intersection is two way points ahead of us
        wp_cross = wp_cross.next(2)[0]  # returning waypoint id
...
def generate_target_waypoint_list(waypoint, turn=0):
    ...
    while True:
        ...
        if len(wp_choice) > 1:  # we have more choice meaning we at intersection
        ...
        if turn != 0 and reached_junction and len(plan) >= 3:
            ...
            v_2 = vector(
                ...
                plan[-2][0].transform.location)
            angle_wp = math.acos(
                np.dot(v_1, v_2) / abs((np.linalg.norm(v_1) * np.linalg.norm(v_2))))  # a · b = |a| × |b| × cos(θ) thats why acos i.e., cos-1
            if angle_wp < threshold:  # way points are in a straight line infront of each other
                break
...
def choose_at_junction(current_waypoint, next_choices, direction=0):
    ...
    for waypoint in next_choices:
        waypoint = waypoint.next(10)[0]  # so we getting a list of three waypoints if we have three turns available on the intersection!!
        '''
        Returns a list of waypoints at a certain approximate distance from the current one. 
        It takes into account the road and its possible deviations without performing any lane change and returns one waypoint per option. 
        ...
        cross = np.cross(current_vector, select_vector)[2]  # the 2 is z axis value and is 0, 1, or -1 right? -1 means the last way point as -1 index in array and 0,1 are the first two indices
        #If vectors in same direction or 180 degree, cross product "z value i.e., [2] index" is zero. and -1 or 1 for left or right
        cross_list.append(cross)  # -1, 1, 0 ?
        cross_to_waypoint[cross] = waypoint
    select_cross = None
    if direction > 0:
        select_cross = max(cross_list)  # 1
    elif direction < 0:
        select_cross = min(cross_list)  # -1
    else:
        select_cross = min(cross_list, key=abs)  # 0
