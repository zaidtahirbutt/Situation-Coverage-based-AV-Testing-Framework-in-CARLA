...
class KeepVelocity(AtomicBehavior):
    ...
    def __init__(self, actor, target_velocity, other_veh_agentZ, other_vehicle_goal_carla_Location, duration=float("inf"), distance=float("inf"), name="KeepVelocity"):
        """
        Setup parameters including acceleration value (via throttle_value)
        and target velocity
        """
        ...
        # maybe destroy the sensors of other veh agentZ here? Since it is a copy?
        # No that would lead to sensors of the copy getting destroyed also since we are using the same carla serve and same other veh id
        # destroying the sensors of self.other_veh_agentZ_copy sensors would destroy the sensors of other_veh_agentZ in the carla server since 
        # both have the same other veh id and are connecting to same carla serve



        self.other_veh_goal_loc = other_vehicle_goal_carla_Location
        self.high_velocity_value = target_velocity #30 #target_velocity #10  #target_velocity  #15  #40
    ...
    def update(self):
        ...
        if self.flag_controller == False:
            self.other_veh_agentZ_copy.agent.set_destination(self._location, self.other_veh_goal_loc, clean=True)
        
        self.flag_controller = True
...
class ActorDestroy(AtomicBehavior):
    ...
    #def __init__(self, actor, other_veh_agentZ, name="ActorDestroy"):
    def __init__(self, actor, name="ActorDestroy"):
        ...
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        #self.other_veh_agentZ_copy2 = other_veh_agentZ # Modification

    def update(self):
        new_status = py_trees.common.Status.RUNNING
        if self._actor:
            # Modification
            #self.other_veh_agentZ_copy2.game_loop_end()
