...
class AtomicCondition(py_trees.behaviour.Behaviour):
    ...
    *All behaviors should use this class as parent*
    ...
    def __init__(self, name):
        ...
        """
        super(AtomicCondition, self).__init__(name)  # py trees behaviour
...
class StandStill(AtomicCondition):
    ...
    def update(self):
        ...
        if velocity > EPSILON:
            self._start_time = GameTime.get_time()  # catch up to gametime till v = 0
...
class InTriggerRegion(AtomicCondition):
    ...
    def __init__(self, actor, min_x, max_x, min_y, max_y, name="TriggerRegion"):
        ...
        """
        super(InTriggerRegion, self).__init__(name)  # SUPERING ATOMIC CONDITION WTF
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
        self._min_x = min_x
        self._max_x = max_x
    ...
    def update(self):
        ...
        not_in_region = (location.x < self._min_x or location.x > self._max_x) or (
            location.y < self._min_y or location.y > self._max_y)  # true for inital region of ego due to y position
...
class InTimeToArrivalToLocation(AtomicCondition):
    ...
    def update(self):
        ...
        if velocity > EPSILON:  # 0.001
            time_to_arrival = distance / velocity  # v is almost zero
