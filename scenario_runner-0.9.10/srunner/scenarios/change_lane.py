...
class ChangeLane(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        tesla_driving_fast = WaypointFollower(self.other_actors[0], self._fast_vehicle_velocity)  # OpenScenario: The WaypointFollower atomic must be called with an individual name if multiple consecutive WFs. ?? This  py_trees.composites.Parallel("DrivingTowardsSlowVehicle", name?
        ...
        sequence_tesla.add_child(just_drive)  # adding parallel node in pytree_sequence()

        # change lane: AFTER getting close to a certain distance to ther other vehicle 1, the other vehicle 0 will change lane 
        lane_change_atomic = LaneChange(self.other_actors[0], distance_other_lane=200)  # s/o waypoint follower atomic (class), both son and daddy are run in parallel to drive the vehicle
        sequence_tesla.add_child(lane_change_atomic)
        sequence_tesla.add_child(Idle())  # remain stuck in here till the other pytree in parall effs shiz up!
        ...
        endcondition_part1 = InTriggerDistanceToVehicle(self.other_actors[1],
                                                        self.ego_vehicles[0],
                                                        distance=20,
                                                        name="FinalDistance")  # this gona eff shiz up
        endcondition_part2 = StandStill(self.ego_vehicles[0], name="FinalSpeed")  # inf in this man wtf
        ...
        root.add_child(endcondition)  # here. xP xD
        return root  # parallel mega tree
