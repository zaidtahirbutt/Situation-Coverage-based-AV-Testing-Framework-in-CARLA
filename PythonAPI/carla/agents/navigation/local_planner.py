...
class LocalPlanner(object):
    ...
    def _init_controller(self, opt_dict):
        ...
        self._compute_next_waypoints(k=200)  # cleared anyway later
    ...
    def run_step(self, debug=False):
        ...
        #print(f"{self._current_waypoint} is local start waypoint")
        #print(f"{self.target_waypoint} is local target waypoint (not end waypoint yameero)")
        #sys.exit("whatt")
        ...
        Waypoint(Transform(Location(x=13.992948, y=134.392670, z=0.000000), Rotation(pitch=360.000000, yaw=-0.817200, roll=0.000000))) is local start waypoint
        Waypoint(Transform(Location(x=5.511875, y=135.091751, z=0.000000), Rotation(pitch=360.000000, yaw=269.637451, roll=0.000000))) is local target waypoint (not end waypoint yameero)
        '''
        ...
        for i, (waypoint, _) in enumerate(self._waypoint_buffer):
            if waypoint.transform.location.distance(vehicle_transform.location) < self._min_distance:
                max_index = i  # jump to this waypoint directly as vehcicle has moved to this waypoint after control loop :3
        if max_index >= 0:
            for i in range(max_index + 1):  # range(3) is 0, 1, 2. That means we have to add 1 for max_index = i if i =3
