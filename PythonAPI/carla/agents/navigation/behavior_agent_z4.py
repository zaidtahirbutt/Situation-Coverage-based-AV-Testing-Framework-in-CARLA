...
from agents.navigation.types_behavior2 import Cautious, Aggressive, Normal

from agents.tools.misc import get_speed, positive

class BehaviorAgentZ4(Agent):
    """
    ...
    def __init__(self, vehicle, ignore_traffic_light=False, behavior='normal', target_velocity=40):
        """
        Constructor method.
        ...
        super(BehaviorAgentZ4, self).__init__(vehicle)
        self.vehicle = vehicle
        self.ignore_traffic_light = ignore_traffic_light
        self._local_planner = LocalPlanner(self)  # def __init__(self, agent):
        self._grp = None
        ...
        if behavior == 'cautious':  # we are using cautious right now
            self.behavior = Cautious()  # object with different parameters hard coded.

            # Cautious max speed is 40
    ...
    def update_information(self, world, target_velocity):
        """
        ...
        #self.speed_limit = world.player.get_speed_limit()  # from sign posts, # self._target_speed = speed (the speed_limit)
        


        # *************************************** ADDED ********************
        #self.speed_limit = world.player.get_speed_limit()  # from sign posts, # self._target_speed = speed (the speed_limit)
        self.speed_limit = target_velocity  # 100 changed # Added
        


        # though we have max speed variable for our behaviour agent behaviour object as well
        ...
        Return: float – m/s
        '''
        # *********************** SPEEEEEEEEEEEED LIMIT HERE!!!!!!!!!!!!!!!!!!!! **************
        self._local_planner.set_speed(self.speed_limit)  # self._target_speed = speed
        ...
        self.incoming_waypoint, self.incoming_direction = self._local_planner.get_incoming_waypoint_and_direction(
            steps=self.look_ahead_steps)  # Returns direction and waypoint at a distance ahead defined by the user.
    ...
    def set_destination(self, start_location, end_location, clean=False):
        ...
        """
        if clean:  # is True in main script
    ...
    def reroute(self, spawn_points):
        ...
        random.shuffle(spawn_points)  # yas!!!!! #shuffled here
    ...
    def _overtake(self, location, waypoint, vehicle_list):
        ...
        left_turn = waypoint.left_lane_marking.lane_change  # possible or nah
        right_turn = waypoint.right_lane_marking.lane_change  # same

        left_wpt = waypoint.get_left_lane()  # left lane waypoint
        ...
        if (left_turn == carla.LaneChange.Left or left_turn ==
            ...
            new_vehicle_state, _, _ = self._bh_is_vehicle_hazard(waypoint, location, vehicle_list, max(
                self.behavior.min_proximity_threshold, self.speed_limit / 3), up_angle_th=180, lane_offset=-1)  # Lane_offset = -1 might be the key, cx it is -1 for left and 1 for right :3  # up angle means I guess to the left or right maybe
            if not new_vehicle_state:  # if no vehicle to the left? I guess. No I don't think so. 
            # okay so we searched the LEFT lane for hazardous vehicles infront because we put lane_offset = -1
            # and if we did not get any hazourd, we enter this condition and set new distination for our local planner, not the global planner :3
            ...
            # overtaken already? No. I think this means that in order to avoid the collision the ego will change lane and this will automatically be it's new start point!!!!! 
            # NOOOOOO. After we set destination and get a new start point the run step method planner of our
            # local planner makes our ego vehicle's first target as the new lane change waypoint, either left
            # or right.  


            # ************ IMP ********************************************************
            #self.target_waypoint, self._target_road_option = self._waypoint_buffer[0]   
            # so self._waypoint_buffer[0] = left_wpt.transform.location and it is the first
            # target for the local planner and the end_waypoint is the last target waypoint
            # from set_destination to the self._local_planner.set_global_plan(route_trace, clean) method
            ...
            if not new_vehicle_state:  # same explanation as for the left overtake
    ...
    def collision_and_car_avoid_manager(self, location, waypoint):
        ...
        if self.direction == RoadOption.CHANGELANELEFT:
            ...
            if vehicle_state and self.direction == RoadOption.LANEFOLLOW and \
                    ...
                    and self.behavior.overtake_counter == 0 and self.speed > get_speed(vehicle):  # overtake counter = 0 for agressive and normal mode
                self._overtake(location, waypoint, vehicle_list)  # loc is ego veh loc and waypoint

            #This condition means that we are going straight and are not on an intersection and car is infront of us, we should overtake it.

            # Check for tailgating

            # if not meaning that if the vehicle is clear of danger but is infront, maybe.
            # like other vehicle is infront and has the same line following opt as us but
            # is at a safe distance. As soon as the distance is not safe, i.e., we are faster,
            # overtaking option kicks in!! 
            # all of these options are for not junction
            # and tailgating is like messing wih the other vehicle so I guess it is for aggressive driving xD
            elif not vehicle_state and self.direction == RoadOption.LANEFOLLOW \
                    and not waypoint.is_junction and self.speed > 10 \
                    and self.behavior.tailgate_counter == 0:  # does not tailgate in aggressive, simply overatkes, the after one tailgate 
                    # method is called, it is updated to 200 and after 200 cycles it goes back to 0, which means the shareef car
                    # tries to get behind the other vehicle to overtake maybe?.. Duno. We'll see if we need it anyway.
    ...
    def run_step(self, debug=False):
        ...
        if walker_state:  # walker (pedestrian) is close Z
            ...
            if distance < self.behavior.braking_distance:  # depends on the behaviour. Lower for agressive behaviour. 
                return self.emergency_stop()

        # 2.2: Car following behaviors
        vehicle_state, vehicle, distance = self.collision_and_car_avoid_manager(  # we are looking for collisions infront of the ego
            ego_vehicle_loc, ego_vehicle_wp)

        """
        We are also checking overtake and tail gaiting and setting them as destinations if need be in local planner
        ...
        # 4: Intersection behavior ************* REDUCING SPEED HERE *****************************
        ...
        #else:  # commented this out because of above comment out
        
        if True: 
            #control = self._local_planner.run_step(
            #    target_speed= min(self.behavior.max_speed-15, self.speed_limit - self.behavior.speed_lim_dist), debug=debug)
            

            #control = self._local_planner.run_step(
            #    target_speed= min(self.behavior.max_speed-15, self.speed_limit), debug=True)


            #Debug = False to stop displaying arrows and load on server
            control = self._local_planner.run_step(
                target_speed= min(self.behavior.max_speed-15, self.speed_limit), debug=False)

            # self.behavior.speed_lim_dist = 6 for cautious
            # We are using cautious behavior and Cautious max speed is 40 
            # I have hard coded the speed limit to be 100 in this script (CHANGED THIS TO 40)

            # *************************************** ADDED ********************
        #self.speed_limit = world.player.get_speed_limit()  # from sign posts, # self._target_speed = speed (the speed_limit)
        #self.speed_limit = 40  # 100 changed # I AM USING THIS ABOVE. Need to change this for targer speed. Look into local planner to confirm :3 
        ...
        if self.incoming_waypoint.is_junction and (self.incoming_direction == RoadOption.LEFT or self.incoming_direction == RoadOption.RIGHT):
            control = self._local_planner.run_step(
                target_speed=min(self.behavior.low_speed, self.speed_limit - 5), debug=debug)
